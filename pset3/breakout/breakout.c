...
{
     GRect rect;
     char*  colorBox[5]={ "GREEN","BLACK","RED" ,"ORANGE" ,"BLUE" } ;
     for (int j = 0; j < ROWS; j++)
     {
        for (int i = 0; i < COLS; i++)
        {
           rect = newGRect(5+i*39, 40+20*j, 35,10);// TODO FUN
           setFilled (rect, true);
           setColor(rect, colorBox[ j ]);
           add(window,rect);
        }
   }
}
...
GOval initBall(GWindow window)
{
    ...
    GOval ball = newGOval(x, y ,25, 25); // TODO FUN
...
GRect initPaddle(GWindow window)
{
    GRect paddle;   // TODO FUN
    paddle = newGRect(165,588,100,10);
...
GLabel initScoreboard(GWindow window)
{
    GLabel score = newGLabel("");   // TODO FUN
    int i = 0;
    char s[3];
