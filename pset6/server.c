...
// TODO      validate request-line
            ...
            if (strncmp(HTTP_version, "HTTP/1.1\r\n", strlen(HTTP_version)) != 0)
            {
                error(505);
                continue;
            }
...
// TODO    // extract query from request-target
            char* queryStart = strchrnul(request_target, '?');
            char* queryEnd = strchr(request_target, '\0');
            char query[strlen(queryStart) + strlen(queryEnd) + 2];
            ...
            {
                char* tempQuery = strstr(request_target, "?");
...
// TODO:    // extract path's extension
            ...
            char* file_name_temp = strstr(absolute_path, "/");
            char* file_name = strcpy(buffer, &file_name_temp[1]);
            char* extension_temp = strchr(file_name, '.');
            char temp_buffer[50];
            char* extension = strcpy(temp_buffer, &extension_temp[1]);
